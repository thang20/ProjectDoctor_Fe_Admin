...
public class AllClient extends AppCompatActivity {
    RecyclerView rcvAllHospital;
    ImageView back;
    ItemNewClientAdapter itemNewClientAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rcvAllHospital = findViewById(R.id.rcv_home_home2_allClient);
        allCl = (Toolbar) findViewById(R.id.tb_allclient);
        setSupportActionBar(allCl);


//        allCl.setSubtitle("Test Subtitle");
//        allCl.inflateMenu(R.menu.main_menu_search);




        back = findViewById(R.id.imv_home_home2_allClient_back);
        back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(AllClient.this, Home.class));
                finish();
            }
        });
        ProgressDialog progressDialog;
        progressDialog = new ProgressDialog(AllClient.this);
        progressDialog.show();
        progressDialog.setContentView(R.layout.process_dialog);
        rcvAllHospital.setLayoutManager(new LinearLayoutManager(AllClient.this));
        ...
        Apiservice.apiservice.allclient(body).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                String notice = "";
                try {
                    notice = response.body().string();
                    List<ItemNewClient> list = new ArrayList<ItemNewClient>();
...
//                        Log.d("data123", obj.toString());
                        ...
                        list.add(new ItemNewClient(name,
                                email, password, phone, image, dateofbirth, gender, address, money, point, carepayactive));

                    }
                    progressDialog.dismiss();
                    itemNewClientAdapter = new ItemNewClientAdapter(AllClient.this ,list);
                    rcvAllHospital.setAdapter(itemNewClientAdapter);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String query) {
                itemNewClientAdapter.getFilter().filter(query);
                return false;
            }

            @Override
            public boolean onQueryTextChange(String newText) {
                itemNewClientAdapter.getFilter().filter(newText);
                return false;
            }
        });
